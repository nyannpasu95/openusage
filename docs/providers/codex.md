...
OhMyUsage floors the remaining credit balance to a whole number and displays its fixed USD
...
When available, OhMyUsage displays the on-demand reset count as the first detail text metric,
...
OhMyUsage Codex plugin auth lookup order:
...
If file-based OAuth credentials are missing, invalid, or fail with an auth/session error during refresh or usage lookup, OhMyUsage tries the macOS keychain fallback. Non-auth usage failures, such as server errors or invalid responses, are shown directly.
